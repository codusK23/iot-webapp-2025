...
namespace MyPortfolioWebApp.Controllers
{
    public class NewsController : Controller
    {
        ...
        public async Task<IActionResult> Index(int page = 1, string search = "")
        {
            ...
            // 최종단계
            //var totalCount = _context.News.FromSql($@"SELECT Id FROM News WHERE Title LIKE '%{search}%'").Count(); // 전체 게시글 수  // 문제 발생!
            var totalCount = _context.News.Where(n => EF.Functions.Like(n.Title, $"%{search}%")).Count(); // HACK! 위 구문이 오류 발생
            ...
            // 저장프로시저 호출
            var news = await _context.News.FromSqlRaw($"CALL New_PagingNews({startCount}, {endCount}, '{search}')").ToListAsync();
            return View(news);
        }
